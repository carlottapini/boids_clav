...
TEST_CASE("means' test") {
  bd::Flock flock;
  flock.all_boids_.push_back(
      bd::Boid(sf::Vector2f(10.f, 20.f), sf::Vector2f(1.f, 0.f)));
  flock.all_boids_.push_back(
      bd::Boid(sf::Vector2f(20.f, 30.f), sf::Vector2f(0.f, 1.f)));
  flock.all_boids_.push_back(
      bd::Boid(sf::Vector2f(30.f, 40.f), sf::Vector2f(1.f, 1.f)));
  ...
  SUBCASE("mean distance's test") {
    ...
    CHECK(mean_distance ==
          doctest::Approx((dist1 + dist2 + dist3) / 3).epsilon(0.01));
  }
}